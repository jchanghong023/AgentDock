...
impl<K: Clone, M> Widget<M, Theme, Renderer> for VirtualList<'_, K, M> {
    ...
    fn draw(&self, tree: &Tree, renderer: &mut Renderer, _theme: &Theme, _style: &renderer::Style,
        ...
        paint::rect(renderer, bounds, style::SURFACE);
        for i in visible_range(self.rows.len(), state.offset, bounds.height, HEIGHT) {
            ...
            let color = if row.selected { style::BLUE } else if row.muted { Color::from_rgb8(70, 91, 131) } else { style::INK };
            let icon = if row.folder { if row.expanded { "▾" } else { "▸" } } else { "" };
            paint::label(renderer, icon, Point::new(x, y), Size::new(14.0, HEIGHT), style::font(), 13.0, color, row_clip);
            if y >= clip.y && y + HEIGHT <= clip.y + clip.height { paint::file_icon(renderer, x + 14.0, y + 6.0, row.folder, row.star == Some(true)); }
            let right = if row.star.is_some() { if state.hover == Some(i) { 62.0 } else { 35.0 } } else { 10.0 };
            let text_clip = Rectangle { x: x + 35.0, y, width: (bounds.x + bounds.width - right - x - 35.0).max(0.0), height: HEIGHT };
            if let Some(text_clip) = text_clip.intersection(&row_clip) {
                paint::label(renderer, &row.label, Point::new(x + 35.0, y), Size::new(10000.0, HEIGHT), if row.folder && row.depth == 0 { style::bold() } else { style::font() }, 13.0, color, text_clip);
            }
            if let Some(starred) = row.star {
                paint::label(renderer, if starred { "★" } else { "☆" }, Point::new(bounds.x + bounds.width - 31.0, y), Size::new(22.0, HEIGHT), Font::DEFAULT, 17.0,
                    if starred { Color::from_rgb8(255, 185, 0) } else { Color::from_rgb8(145, 160, 181) }, row_clip);
            }
