...
impl Engine{
    ...
    pub fn bottom(&mut self){self.offset=0;self.selection=None;self.changed();}
...
#[cfg(test)]mod tests{
    ...
    #[test]fn device_reply(){let(mut e,b)=engine();e.advance(b"\x1b[6n");assert_eq!(&*b.lock().unwrap(),b"\x1b[1;1R");}
    #[test]fn bracketed_paste(){let(mut e,b)=engine();e.advance(b"\x1b[?2004h");e.paste("abc").unwrap();assert_eq!(&*b.lock().unwrap(),b"\x1b[200~abc\x1b[201~");}
