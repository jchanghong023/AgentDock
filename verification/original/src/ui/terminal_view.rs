...
use iced::{Color,Element,Event,Font,InputMethod,Length,Pixels,Point,Rectangle,Renderer,Size,Theme,keyboard,mouse,window};
use iced::input_method::{self,Preedit,Purpose};
...
impl<M>Widget<M,Theme,Renderer> for TerminalView<'_,M>{
    ...
    fn draw(&self,tree:&Tree,r:&mut Renderer,_theme:&Theme,_style:&renderer::Style,layout:Layout<'_>,_cursor:mouse::Cursor,viewport:&Rectangle){
        ...
        for(row,cells)in self.snapshot.lines.iter().enumerate(){
            ...
            for cell in cells{
                let bounds=Rectangle{x:a.x+cell.column as f32*s.width,y,width:cell.width as f32*s.width,height:s.height};
                let Some(crop)=bounds.intersection(&clip)else{continue};
                paint::rect(r,crop,if cell.selected{Color::from_rgb8(51,78,111)}else{paint::rgba(cell.bg)});
                ...
                if !cell.text.is_empty()&&cell.text!=" "{paint::label(r,&cell.text,bounds.position(),bounds.size(),font,self.size,fg,crop);}
