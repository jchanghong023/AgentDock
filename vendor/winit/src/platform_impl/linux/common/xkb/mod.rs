...
impl Context {
    ...
    fn new_inner(local_fallback: bool) -> Result<Self, Error> {
        if xkb::xkbcommon_option().is_none() {
            return Err(Error::XKBNotFound);
        }

        let context = XkbContext::new(local_fallback)?;
    ...
    pub fn from_x11_xkb(xcb: *mut xcb_connection_t) -> Result<Self, Error> {
        if xkb::x11::xkbcommon_x11_option().is_none() {
            return Err(Error::XKBNotFound);
        }
        let result = unsafe {
    ...
    #[cfg(x11_platform)]
    pub fn from_local_keymap() -> Result<Self, Error> {
        let mut this = Self::new_inner(true)?;
        // Keep an explicit user override authoritative. Otherwise include the
        // CentOS system data path even when the library was built elsewhere.
        if std::env::var_os("XKB_CONFIG_ROOT").is_none() {
            unsafe {
                (XKBH.xkb_context_include_path_append)(
                    this.context.as_ptr(), b"/usr/share/X11/xkb\0".as_ptr().cast());
            }
        }
        let keymap = XkbKeymap::from_names(&this.context).ok_or(Error::XKBNotFound)?;
        let state = XkbState::new_wayland(&keymap).ok_or(Error::XKBNotFound)?;
        this.core_keyboard_id = -1;
        this.state = Some(state);
        this.keymap = Some(keymap);
        Ok(this)
    }
...
impl XkbContext {
    pub fn new(local_fallback: bool) -> Result<Self, Error> {
        // A relocated library may have no valid compiled-in data paths. Create
        // the fallback context first so that we can append the system path.
        let flags = if local_fallback {
            xkb_context_flags::XKB_CONTEXT_NO_DEFAULT_INCLUDES
        } else {
            xkb_context_flags::XKB_CONTEXT_NO_FLAGS
        };
        let context = unsafe { (XKBH.xkb_context_new)(flags) };
        ...
        if local_fallback {
            unsafe { (XKBH.xkb_context_include_path_append_default)(context.as_ptr()); }
        }
        Ok(Self { context })
    }
}
