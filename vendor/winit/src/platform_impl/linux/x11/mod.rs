...
impl<T: 'static> EventLoop<T> {
    pub(crate) fn new(xconn: Arc<XConnection>) -> Result<EventLoop<T>, EventLoopError> {
        ...
        let xkbext = xconn
            ...
            .map_err(|_| os_error!(OsError::Misc("Failed to query XKB extension")))?;
        ...
        let xkb_context = if xkbext.is_some() {
            Context::from_x11_xkb(xconn.xcb_connection().get_raw_xcb_connection())
        } else {
            warn!("XKEYBOARD unavailable; using local XKB keymap (XKB_DEFAULT_LAYOUT, default US)");
            Context::from_local_keymap()
        }.map_err(|_| os_error!(OsError::Misc(
            "Keyboard initialization failed: check libxkbcommon and XKB_CONFIG_ROOT (normally /usr/share/X11/xkb), including rules/evdev"
        )))?;
        ...
        if xkbext.is_some() {
            xconn
                ...
                .map_err(|_| os_error!(OsError::Misc("Failed to select XKB events")))?;
        }
