...
pub struct EventProcessor {
    ...
    pub xkbext: Option<ExtensionInformation>,
    pub target: RootAEL,
...
impl EventProcessor {
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        // Core X11 auto-repeat sends a release/press pair with identical time
        // and keycode. Keep the key held so the following press is a repeat.
        if self.xkbext.is_none() && event_type == xlib::KeyRelease {
            let wt = Self::window_target(&self.target);
            unsafe {
                if (wt.xconn.xlib.XPending)(wt.xconn.display) > 0 {
                    let mut next: XEvent = std::mem::zeroed();
                    (wt.xconn.xlib.XPeekEvent)(wt.xconn.display, &mut next);
                    if next.get_type() == xlib::KeyPress {
                        let released: &XKeyEvent = xev.as_ref();
                        let pressed: &XKeyEvent = next.as_ref();
                        if released.keycode == pressed.keycode
                            && released.time == pressed.time
                            && released.window == pressed.window
                        {
                            return;
                        }
                    }
        ...
        match event_type {
            ...
            _ => {
                if self.xkbext.as_ref().is_some_and(|ext| event_type == ext.first_event as _) {
    ...
    {
        let wt = Self::window_target(&self.target);
        if self.xkbext.is_none() {
            let reply = wt.xconn.xcb_connection().query_pointer(u64::from(window_id) as u32)
                .ok().and_then(|cookie| cookie.reply().ok());
            if let Some(reply) = reply {
                self.update_mods_from_core_event(window_id, reply.mask.into(), &mut callback);
            }
            return;
        }
