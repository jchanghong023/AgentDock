...
pub fn present(
    ...
    if damage.is_empty() {
        ...
        let damage = bounded_damage(damage::group(
            damage,
...
// Each damage region traverses all layers and updates a viewport-sized clip
// mask. Terminal menus can produce hundreds of regions even after grouping.
// Bound that repeated work while preserving precise small updates. A bounding
// rectangle covers every dirty pixel; the normal renderer repaints its gaps.
fn bounded_damage(regions: Vec<Rectangle>) -> Vec<Rectangle> {
    const MAX_REGIONS: usize = 8;
    if regions.len() <= MAX_REGIONS {
        return regions;
    }
    let bounds = regions.iter().skip(1).fold(regions[0], |bounds, region| bounds.union(region));
    vec![bounds]
}
...
mod damage_tests {
    ...
    #[test]
    fn small_updates_keep_their_regions() {
        assert!(bounded_damage(vec![]).is_empty());
        let regions: Vec<_> = (0..8).map(|i| Rectangle {
            x: i as f32 * 20.0, y: 10.0, width: 5.0, height: 7.0,
        }).collect();
        assert_eq!(bounded_damage(regions.clone()), regions);
    }

    #[test]
    fn fragmented_updates_cover_every_dirty_pixel() {
        let regions: Vec<_> = (0..600).map(|i| Rectangle {
            x: (i % 40) as f32 * 20.0, y: (i / 40) as f32 * 21.0,
            width: 9.0, height: 21.0,
        }).collect();
        let result = bounded_damage(regions.clone());
        assert_eq!(result.len(), 1);
        for region in regions { assert!(region.is_within(&result[0])); }
        assert_eq!(result[0], Rectangle { x: 0.0, y: 0.0, width: 789.0, height: 315.0 });
    }
}
